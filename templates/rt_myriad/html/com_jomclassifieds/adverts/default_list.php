...
<form action="<?php echo JURI::getInstance()->toString(); ?>" method="post" name="jcsForm" id="jcsForm" enctype="multipart/form-data">
   <?php if ($this->params->get('show_page_heading')) : ?>
   		<div class="jomcl-header-text">
			...
			<?php if($config->rss != 'none'){ ?>
                <span class="jomcl-rss"><?php echo $this->buidRSS();?></span>	
            <?php } ?>
    	</div>
   <?php endif; ?>
...
<div class="listview">
	...
	foreach($items as $item) :
		...
		$user = JFactory::getUser($item->userid); //echo $item->userid;
      //  if(isset($user) && $user !='')		
      ...
      <div class="rt-grid-12 full-title-show">
            <h4>
			 ...
			 <?php } ?>
			  ...
			  <?php } ?><strong style="font-size: 14px;">AD ID: <?php echo $item->id; ?></strong>
            </h4>
           </div>
		  <div class="rt-grid-4">
			  <div class="detai">
				  ...
				  <div class="cat-detials">
					<?php 	
						//if($item->category)	echo '<strong class="adtype-label">'.$item->category.'</strong><br>'; 
		  ...
		  <div class="rt-grid-2 contacts-info">
			  <?php if(!$item->expired) :  ?>
